...
class MasterViewController: UITableViewController {
    ...
    override func prepare(for segue: UIStoryboardSegue, sender: Any?) {
        ...
        let detailViewController = navigationController.topViewController as? DetailViewController {
            ...
            case "EQUIPMENT":
                detailViewController.request = requestParamsWithAlert("roomid", "siteid")
            case "FREE ROOMS":
                detailViewController.request = requestParamsWithAlert("start_datetime", "end_datetime")
            case "PEOPLE":
                detailViewController.request = requestParamsWithAlert("query")
            case "PERSONAL TIMETABLE":
                detailViewController.request = requestParamsWithAlert("client_secret")
            case "TIMETABLE BY MODULES":
                detailViewController.request = requestParamsWithAlert("client_secret", "modules")
            default:
                detailViewController.request = [:]
            }
        }
    }
}
